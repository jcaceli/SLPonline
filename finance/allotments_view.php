...
if (isset($_GET['id'])) {
 // $_GET  = filter_input_array(INPUT_GET, FILTER_SANITIZE_NUMBER_INT);
  $stmt = $db->prepare("SELECT fa.region as region,fa.type as type,fa.subtype as subtype,fa.saa as saa,fa.uacs as uacs,fa.fundsource as fundsource,fa.fundsourceyear as fundsourceyear,fa.amount as amount,fa.dateadded as dateadded,fa.d8 as d8, hrd.firstname as fn, hrd.id as id,fa.allotid as allotid from fin_allotments as fa LEFT JOIN HRDB as hrd ON fa.hrdbid=hrd.id  WHERE allotid = :id");
     ...
     <div class="row">
          <table class="table table-bordered" style="line-height:0.9;vertical-align:middle;border:0;margin-bottom:1em">
              ...
              <tr>
                ...
                <td><?php echo $rowdv['amount'];?></td>
                <td><?php echo $rowdv['d8'];?></td>
                <td><a href="../hr/user.php?id=<?php echo $rowdv['id']; ?>" style="color:#00ADDe"><?php echo $rowdv['fn']; ?></a></td>
                <td><?php echo $rowdv['dateadded']; ?></td>
              </tr>
              <!--comments-->     
              </table>
